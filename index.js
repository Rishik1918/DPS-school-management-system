...
document.getElementById('loginEmail').addEventListener('keyup', ()=>{
    ...
    error_msg.innerHTML = '';
    error_msg.style.display = 'none'; // Added: Ensure the message itself is hidden
});
...
function submitForm(event) {
    ...
    .then(data => {
        ...
        error_msg.style.display = 'block'; // Added: Make the alert message visible
    })
    .catch(error => {
...
function hideLoginForm(hide){
    ...
    document.getElementById("error-msg").style.display = 'none'; // Added: Hide the message element
...
document.getElementById('forgotPassword-form').addEventListener('submit', (event)=>{
    ...
    .then(data => {
        ...
        if(data['status'] === 'success'){
            ...
            document.getElementById("error-msg").style.display = 'block'; // Added: Make the alert message visible
        }

        })
        .catch(error => {
            console.error("Error:", error);
            document.getElementById("error-msg").innerHTML = 'Network error or server unavailable.';
            document.querySelector('.alert-box').style.display = 'block';
            document.getElementById("error-msg").style.display = 'block'; // Added: Make the alert message visible
            document.getElementById('backToLogin').style.display = 'block';
            document.getElementById('forgotEmail').disabled = false;
...
document.getElementById('resendOTP').addEventListener('click', function(){
    ...
    .then(data => {
        ...
        document.getElementById('resendOTP').innerHTML = 'OTP sent'; // Changed to "OTP sent"
        setTimeout(()=>{
...
document.getElementById('createNewPassword-form').addEventListener('submit', (event)=>{
        ...
        if(newPassword === confirmPassword){
            ...
            .then(data => {
                ...
                if(data['status'] === 'update_success'){
                    ...
                    errorBox.style.display = 'block';
                    error_message.style.display = 'block'; // Added: Make the alert message visible


                   setTimeout(() => {
                    // Reset error box after successful update and redirect
                    error_message.classList.remove('alert-danger', 'alert-success');
                    error_message.innerHTML = '';
                    errorBox.style.display = 'none';
                    error_message.style.display = 'none'; // Added: Hide the message element

                    window.location.href = './'; // Redirect to login page
                   }, 1000);


                }else{ // Handle update_success errors (e.g., if database update fails)
    ...
    // This section seems redundant as it duplicates the form data creation and fetch call
    // It's likely leftover or intended for a different validation.
    // The main logic is within the if(isStrongPassword()){...} block.
    // let newPassword = document.getElementById('newpassword').value;
    // let confirmPassword = document.getElementById('confirmpassword').value;
    // let sendData = new FormData();
...
function isStrongPassword() {
    document.getElementById('passwordNotSame').style.display = 'none';
      ...
      if (!(password.length >= minLength)) {
          weakBadge.innerHTML = "Password must contain a minimum of 8 characters.";
        ...
        weakBadge.innerHTML = "Password must contain at least one special character (!, %, &, @, #, $, ^, *, ?, _, ~).";
        weakBadge.style.display = 'block';
        return false;
      }
      else{
